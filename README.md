...
- `data/mechanics/` — レジストハック・必要魔命・コンテンツレベル(CL)などのゲーム
  システム解説
...
作成し、`verified: false` などで要検証であることを明示しています。以下は一次情報に
基づく信頼度の高いデータです。

- `data/monsters/resistance_table.json`、`data/combat_formulas.json`、
  `data/encounters/` — 以前にユーザー自身が作成した検証済みのArtifact(実測データ・
  実戦ログに基づく)から抽出
- `data/mechanics/`、`docs/windower.md`の一部セクション — 別のClaudeセッションが
  Web上のWindower Wiki・攻略ブログ・Discord情報等を収集して共有したもの(出典URLを
  各ファイルに明記)
